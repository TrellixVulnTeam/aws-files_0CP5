...
def get_average_box_office_collections():
    try:
        m= Movie.objects.aggregate(avg_collections=Avg('box_office_collection_in_crores'))
        return round(m['avg_collections'],3)
    except: return 0
    
# Task-2
def get_movies_with_distinct_actors_count():
    return list(Movie.objects.annotate(actors_count=
        Count('cast__actor',distinct=True)
        ))
        
# Task-3
def get_male_and_female_actors_count_for_each_movie():
    return Movie.objects.annotate(male_actors_count=
        Count('cast__actor',filter=Q(actors__gender='MALE'),distinct=True),
        female_actors_count=Count('cast__actor',filter=Q(actors__gender='FEMALE'),distinct=True)
        )
...
def get_role_frequency():
    cast_role=Cast.objects.values('role').annotate(
        count=Count('actor',distinct=True)
        )
    d={}
    for cast in cast_role:
        d[cast['role']]=cast['count']
    return d
    
# Task-6
def get_role_frequency_in_order():
    cast_role=Cast.objects.values_list('role').annotate(
        count=Count('role')
        ).order_by('-movie__release_date')
    return list(cast_role)
...
def get_movies_with_atleast_forty_actors():
    return Movie.objects.annotate(
        actors_count=Count('cast__actor',distinct=True)
        ).filter(actors_count__gte=40)
        
# Task-9
def get_average_no_of_actors_for_all_movies():
    try:
        avg=Movie.objects.annotate(
            actors_count=Count('cast__actor',distinct=True)
            ).aggregate(actors_avg=Avg('actors_count'))
        return round(avg['actors_avg'],3)
    except: return 0