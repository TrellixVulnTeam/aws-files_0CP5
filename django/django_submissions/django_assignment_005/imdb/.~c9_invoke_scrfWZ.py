...
def get_rating_object(movie_obj):
    return Rating.objects.select_related('movie').filter(movie=movie_obj)

def get_average_rating_of_movie(movie_obj):
    rating= get_rating_object(movie_obj)
    try:
        avg=(
            1*rating[0].rating_one_count
            +2*rating[0].rating_two_count
            +3*rating[0].rating_three_count
            +4*rating[0].rating_four_count
            +5*rating[0].rating_five_count
            )/(
                rating[0].rating_one_count
                +rating[0].rating_two_count
                +rating[0].rating_three_count
                +rating[0].rating_four_count
                +rating[0].rating_five_count
            )
        return avg
    except : return 0
...
def get_total_rating_of_movie(movie_obj):
    rating= get_rating_object(movie_obj)
    try:
        avg = rating[0].rating_one_count + rating[0].rating_two_count + rating[0].rating_three_count + rating[0].rating_four_count + rating[0].rating_five_count
        return avg
    except: return 0
...
def get_movies_by_given_movie_names(movie_names):
    ...
    for movie in movies:
        cast_list=[]
        for cast in Cast.objects.select_relatedfilter(movie=movie):
            cast_dict={
                'actor':{
