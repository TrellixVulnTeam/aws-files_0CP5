...
class ComplexNumber:
    ...
    def __truediv__(self,other):
        ...
        return ComplexNumber((((a*c)+(b*d))/((math.pow(c,2))+(math.pow(d,2)))),(((b*c)+(a*d))/((math.pow(c,2))+(math.pow(d,2)))))
    
    def __abs__(self):
        absolute=math.sqrt(math.pow(self.real_part,2)+math.pow(self.imaginary_part,2))
        return "{}".format(round(absolute,3))
